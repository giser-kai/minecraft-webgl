...
class ParticleEmitter {
  ...
  setParameters(params) {
    ...
    this.material = new THREE.ShaderMaterial({
      uniforms: {
        textureMode: { value: this.texture }
      },
      vertexShader,
      fragmentShader,
      transparent: true,
  ...
  createParticle() {
    ...
    if(this.positionShape == Shape.CUBE) {
      particle.position = this.randomVector3(this.position, this.positionRange)
    }
    ...
    if(this.velocityShape == Shape.CUBE) {
      particle.velocity = this.randomVector3(this.velocity, this.velocityRange)
    }

    if(this.velocityShape == Shape.SPHERE) {
      const direction = new THREE.Vector3().subVectors(particle.position, this.position)
      const speed = this.randomValue(this.speed, this.speedRange)
      particle.velocity = direction.normalize().multiplyScalar(speed)
    }

    particle.acceleration = this.randomVector3(this.acceleration, this.accelerationRange)
    
    particle.angle = this.randomValue(this.angle, this.angleRange)
    particle.angleVelocity = this.randomValue(this.angleVelocity, this.angleVelocityRange)
    particle.angleAcceleration = this.randomValue(this.angleAcceleration, this.angleAccelerationRange)

    particle.size = this.randomValue(this.size, this.sizeRange)

    const color = this.randomVector3(this.color, this.colorRange)
    particle.color = new THREE.Color().setHSL(color.x, color.y, color.z)

    particle.opacity = this.randomValue(this.opacity, this.opacityRange)
    particle.age = 0
